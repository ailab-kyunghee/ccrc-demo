...
def parse_args():
    ...
    parser.add_argument('--example_root', default='./images/Sample_images', help='Path to D_probe')
...
def concept_attribution_maps(cmaps, args, model, example_loader, num_top_neuron=5, percentile=90, alpha=0.7, gt=False):
    ...
    for j, (img, label) in enumerate(tqdm(example_loader)):
        ...
        if gt:
            plt.savefig(f'{args.heatmap_save_root}/{label.item():04d}/class_attribute_n{num_top_neuron}_p{percentile}_a90/Class_att_gt{label.item():04d}_{(j):02d}.jpg')
        else:
            plt.savefig(f'{args.heatmap_save_root}/{label.item():04d}/class_attribute_n{num_top_neuron}_p{percentile}_a90/Class_att_{predict:04d}_{(j):02d}.jpg')
        plt.clf()
